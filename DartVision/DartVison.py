...
def GetField(image):
    ...
    #j = 4
    #print("Geworfene Zahl:" + str(black_contours[j].points))
    #print("Geworfene Zahl:" + str(white_contours[j].points))
    #print("Geworfene Zahl:" + str(red_contours[j].points))
...
def GetPictureStream(field_contours, channel, commandQueue):
    ...
    with PiCamera() as camera:
        ...
        for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
            ...
            if key == ord("q") or not commandQueue.empty():
                break
...
def commandWorker(commandQueue, channel):
...
if __name__ == '__main__':
    ...
    commandQueue = Queue()
    Process(target=commandWorker, args=(commandQueue, channel)).start()

    # Execute commands
    for command in iter(commandQueue.get, 'STOP'):
        if command == 'light_on':
            light(True)
